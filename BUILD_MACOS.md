...
| The program doesn't quit: its window closes, but it stays in the Dock | A worker thread is stuck in a library call. Usually it's rig control waiting on a CAT server that no longer answers (FLRig waiting on a radio that was switched off or dropped off USB), or audio waiting on the audio server (an unanswered microphone permission prompt). Since rc09 the program waits at most 15 s for rig control and 5 s for audio, then leaves them and exits. Started from a terminal, it prints `Rig control did not stop within 15 s, leaving it` (or `Audio …`). Restart FLRig once the radio is back. How to reproduce it is described in `thread_shutdown.hpp`. |
