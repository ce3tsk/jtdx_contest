...
public:
  ...
  /* CE3TSK: special operating activity, selected on the Special operating tab. The full
     WSJT-X set is declared so that stored values keep their meaning if more are
     implemented later; only NONE and WW_DIGI are offered in the dialog today, the rest are
     shown disabled. Stored as an integer, so do not renumber existing entries. */
  ...
  /* CE3TSK: the grid highlighting the operator actually chose. While a contest owns those
     settings newGrid() reports the forced value, which would otherwise make the auto
     sequencer behave as if wanted-criteria had been set when the operator set none. */
  bool newGridUserChoice () const;
  SpecialOperatingActivity special_op_id () const;
  bool wwDigi () const;   /* CE3TSK: convenience, special_op_id () == WW_DIGI */
  QString special_op_name () const;   /* CE3TSK: contest name for the log comment, empty when none */
  QString special_op_cq_direction () const;   /* CE3TSK: directional CQ a contest calls with */
  int special_op_points (int distanceKm) const;   /* CE3TSK: QSO points, 0 when no contest */
  ...
  FrequencyList_v2 * base_frequencies ();   /* CE3TSK: the everyday list, never the contest one */
...
/* CE3TSK: contest scoring, free so that DisplayText can score a decode without holding a
   Configuration pointer - it deliberately copies what it needs in setConfiguration() rather
   than keeping one. Configuration::special_op_points() delegates to contest_points(). */
int contest_points (Configuration::SpecialOperatingActivity, int distanceKm);

/* CE3TSK: the points tag the message window puts in front of the country name while a
   contest runs: " 1p" .. " 7p", and " --" of the same width when nothing is scored (no grid
   in the message, so the distance is unknown) - the country column stays aligned instead
   of shifting left on those lines, and the dash says "not scored" at a glance. Only meaningful during a contest; the caller
   decides whether to add it at all. */
QString contest_points_tag (int points);

/* Great circle km between two grids, or -1 when it cannot be known. -1 rather than 0
   because 0 km is a real answer, a station in our own square, and scores a point.
...
/* CE3TSK: the separate log a contest keeps. contest_log_filename() is empty when no contest
   is running. The period is derived from the QSO date, never from when contest mode was
   switched on, so enabling and disabling it mid contest cannot move the boundary. */
QString contest_log_slug (Configuration::SpecialOperatingActivity);
QString contest_period (Configuration::SpecialOperatingActivity, QDateTime const& when);
QString contest_log_filename (Configuration::SpecialOperatingActivity, QDateTime const& when);
