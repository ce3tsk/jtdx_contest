...
/* CE3TSK: drop a station from the sequencer's candidate lists while leaving the operator's own
   blacklist entry alone - remove() erases that too. The contest's five minute skip fires by
   itself, every pass, so it must not quietly undo a deliberate Ctrl + right click. */
int QsoHistory::forget(QString const& callsign)
{
  int ret=0;
  if (_working) {
    ret=_calldata.remove(Radio::base_callsign (callsign));
...
/* CE3TSK: WW Digi contest mode. Deliberately not guarded by _working, it is a setting and
...
void QsoHistory::message(QString const& callsign, Status status, int priority, QString const& param, QString const& tyyp, QString const& continent, QString const& mpx, unsigned time, QString const& rep, int freq, QString const& mode)
{
    if (_working)
    {
      /* CE3TSK: WW Digi contest - "CQ WW" names the contest, it is not a geographic
         direction. Left as a directive it matches no continent or prefix and autoselect
         silently skips every station calling CQ WW, ours included. */
      QString tyyp_ = tyyp;
      if (_wwDigi && tyyp_ == "WW") tyyp_ = "";
      if (status == SCQ)
        {
          _CQ.call = callsign;
          _CQ.status = status;
  ...
  //                if (t.status <= SREPORT)
                  if (t.status <= SCQ || t.status == SCALL || (t.status == SREPORT && t.srx_p < RREPORT) || 
                      ...
                      (t.status > SCQ && t.status < SRR73 && ((t.time > 300 && t.time - t.b_time > 300) || (t.time <= 300 && t.b_time - t.time < 86100)))) // an attempt to support CQ and any other message reception from MSHV multislot operation mode
                    {
                      ...
                      if (t.continent == "") t.tyyp = tyyp_; /* CE3TSK */
              ...
              case RCALL:
                {
                  ...
                  t.b_time = time;
                  t.s_rep = rep;
                  /* CE3TSK: WW Digi contest - no signal report is ever received, but the
                     RRR/RR73/73 states refuse to advance while r_rep is empty. Record the
                     SNR we measured of his signal rather than his grid: the grid is already
                     kept in t.grid for the ADIF gridsquare, and this way rst_rcvd gets a
                     real signal value instead of a locator. Set here beside s_rep, not in
                     the param branch above: the value is the SNR and has nothing to do with
                     whether he sent a grid. */
                  if (_wwDigi) t.r_rep = rep;
                  t.rx = freq;
  //                max_r_time = time;
                  as_active = true;
                  break;
  ...
  //                if (t.status <= SCALL ) {
                    ...
                    t.b_time = time;
                    t.tx = freq;
                    /* CE3TSK: WW Digi contest - nothing to record here. s_rep must keep the
                       SNR measured of his signal so that it reaches the log as rst_sent;
                       the grid we sent is our own and is logged as my_gridsquare. */
  //                }
                  break;
                            }
              case RREPORT:
              ...
              case RRREPORT:
                {
                  t.srx_p = t.srx_c;
                  t.srx_c = status;
                  /* CE3TSK: WW Digi contest - our exchange goes out in Tx1/Tx2 as a bare
                     grid, which is recorded as SCALL, so requiring SREPORT here would
                     reject his "R GRID" and stall the QSO. */
                  if (t.status < FIN && (t.stx_c >= SREPORT || (_wwDigi && t.stx_c >= SCALL))) {
                      old_status = t.status;
                      t.status = status;
                      if (priority > t.priority) t.priority = priority;
                      if (t.continent.isEmpty ()) t.continent = continent.trimmed();
                      if (t.mpx.isEmpty ()) t.mpx = mpx;
                      /* CE3TSK: WW Digi contest - param is his 4 character grid, not a
                         signal report, so it goes to t.grid for the ADIF gridsquare while
                         the reports keep the SNR we measured of his signal. s_rep is
                         refreshed here as well because a double click on the callsign wipes
                         the history entry, and nothing else would fill it again in a
                         contest QSO where no report is ever exchanged. */
                      if (_wwDigi) {
                        t.r_rep = rep;
                        t.s_rep = rep;
              ...
              case SRREPORT:
                {
                  ...
                  if(t.status <= SRREPORT) {
                    /* CE3TSK: WW Digi contest - param is the grid we sent, our own exchange,
                       already logged as my_gridsquare. Keep s_rep on the SNR measured of his
                       signal so that rst_sent is a real report. */
                    if (!_wwDigi) t.s_rep = param;
                    old_status = t.status;
                    t.status = status;
                    t.tx = freq;
  ...
  //                max_r_time = time;
            ...
            /* CE3TSK: the QSO's start belongs to this record, and is filled once by the first
               message that engages the station - whatever kind it is. The eight b_time
               assignments above only fire on the opening statuses (SCALL, SREPORT, RCALL and
               friends), so a QSO first seen at RRR73 - a partner picked up late, after the
               sequencer had let go - kept 0, and the logger wrote it as midnight
               (WW_DIGI_CONTEST_SUPPORT 6b: JR5JEU, ZL3TE). Fill-once: it can only fill a gap,
