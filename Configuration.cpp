...
/* CE3TSK: the Fortran geodesic used by grid_distance_km(), same routine MainWindow uses so
   the two can never disagree. Stateful and not reentrant - GUI thread only. */
extern "C" {
...
public:
  ...
  /* CE3TSK: modes_model widened from Modes* to the base type so a filtered proxy can be
     passed - the contest table offers only FT8 and FT4. Safe because the model is used only
     for setModel() and item() reads the mode back by text through Modes::value(). */
  explicit FrequencyDialog (IARURegions * regions_model, QAbstractItemModel * modes_model, QWidget * parent = nullptr)
    : QDialog {parent}
  {
...
private:
  ...
  /* CE3TSK: the contest table's own six. Load, Merge and Reset each filter to FT8 and FT4:
     the mode proxy restricts the cell editor and the insert dialog, but a .qrg file or the
     stock defaults would otherwise put JT65 rows straight into a contest list. */
  void delete_contest_frequencies ();
  void insert_contest_frequency ();
  void load_contest_frequencies ();
  void save_contest_frequencies ();
  void merge_contest_frequencies ();
  void reset_contest_frequencies ();
  ...
  /* CE3TSK: null until MainWindow calls set_jtdxtime() (mainwindow.cpp:457). Left
     uninitialised it is a garbage pointer, and the transceiver thread dereferences it in
     PollingTransceiver::do_post_start() - a crash that only shows outside the normal
     startup order, where the assignment always happens first. */
  JTDXDateTime * jtdxtime_ = nullptr;
  ...
  /* CE3TSK: a contest keeps its own working frequency list, edited on the Contest tab with
     the same table. frequencies() below hands this one out while a contest is selected, so
     everything that asks - the band selector, the validator, best_working_frequency() and
     the band change path - follows without knowing a contest is running. */
  FrequencyList_v2 contest_frequencies_;
  FrequencyList_v2 next_contest_frequencies_;
  ...
  /* CE3TSK: the contest table's own actions and dialog. A separate FrequencyDialog is what
     ...
     and the dialog carries its own mode selector. */
  QAction * contest_frequency_delete_action_;
  QAction * contest_frequency_insert_action_;
  QAction * contest_load_frequencies_action_;
  ...
  /* CE3TSK: special operating activity, and the settings it takes over. While a contest is
     selected the operator's own values are parked in savedSpecialOp_ and handed back when it
     is switched off; specialOpSaved_ guards against a second save capturing the forced
     values and making the restore a no-op. dlgEntrySpecialOp_ is the same set as it stood
     when the dialog was opened, used to preview a return to None before anything has been
     committed - see on_rbSpecialOpNone_toggled().
     The initialisers below are JTDX's own stock defaults, so a set that was never captured
     looks like a fresh install. Every read is guarded by specialOpSaved_ and read_settings()
  ...
  struct SpecialOpSettings
  {
    ...
    /* CE3TSK: new call and its band variants, forced on / on / off like the grid triple: a
       ...
       IS the dupe check and must be lit; per band+mode would show a dupe as new */
    bool newCall = true;
  ...
  SpecialOperatingActivity next_specialOp_; /* CE3TSK */
...
/* CE3TSK */
bool Configuration::newGridUserChoice () const {return m_->specialOpSaved_ ? m_->savedSpecialOp_.newGrid : m_->newGrid_;}
Configuration::SpecialOperatingActivity Configuration::special_op_id () const {return m_->specialOp_;}
bool Configuration::wwDigi () const {return m_->specialOp_ == SpecialOperatingActivity::WW_DIGI;}

/* CE3TSK: the name a contest is known by, used to head the log comment. One line per
   activity as more are supported; an empty string means no contest is running. */
QString Configuration::special_op_name () const
{
  /* every enumerator is listed and there is no default, so -Wswitch warns when an activity
     is added without deciding what it is called in the log. A warning only - the build sets
     -Wall -Wextra but not -Werror - so it has to be read, not relied on to stop anything. */
  switch (m_->specialOp_)
    {
    case SpecialOperatingActivity::WW_DIGI: return QString {"WW-Digi"};
    case SpecialOperatingActivity::NONE:
    case SpecialOperatingActivity::NA_VHF:
    case SpecialOperatingActivity::EU_VHF:
    case SpecialOperatingActivity::FIELD_DAY:
    case SpecialOperatingActivity::RTTY:
    case SpecialOperatingActivity::ARRL_DIGI: break;
    }
  return QString {};
}

/* CE3TSK: the directional CQ a contest is called with - "CQ WW ..." for WW Digi. Transient:
   MainWindow drops it into the Directional CQ box on a change of activity and it is neither
   forced nor greyed afterwards, so it can be typed over. Exhaustive switch as above. */
QString Configuration::special_op_cq_direction () const
{
  switch (m_->specialOp_)
    {
    case SpecialOperatingActivity::WW_DIGI: return QString {"WW"};
    case SpecialOperatingActivity::NONE:
    case SpecialOperatingActivity::NA_VHF:
    case SpecialOperatingActivity::EU_VHF:
    case SpecialOperatingActivity::FIELD_DAY:
...
/* CE3TSK: QSO points for a contest that scores by distance. Exhaustive switch and no
   default, so a contest added without a scoring rule draws -Wswitch. WW Digi scores one
   point per started 3000 km: 1 point below 3000, 2 to 5999, and so on. The maximum falls
   out of the geometry rather than a clamp - azdist returns 20400 km for exact antipodes,
   which is 7, and nothing on Earth is further. */
int contest_points (Configuration::SpecialOperatingActivity activity, int distanceKm)
{
  if (distanceKm < 0) return 0;               // distance not known, so nothing is scored
  switch (activity)
    {
    case Configuration::SpecialOperatingActivity::WW_DIGI: return 1 + distanceKm / 3000;
    case Configuration::SpecialOperatingActivity::NONE:
    case Configuration::SpecialOperatingActivity::NA_VHF:
    case Configuration::SpecialOperatingActivity::EU_VHF:
    case Configuration::SpecialOperatingActivity::FIELD_DAY:
    case Configuration::SpecialOperatingActivity::RTTY:
    case Configuration::SpecialOperatingActivity::ARRL_DIGI: break;
    }
  return 0;
}

QString contest_points_tag (int points)
{
  return points > 0 ? QString {" %1p"}.arg (points) : QString {" --"};
}

int Configuration::special_op_points (int distanceKm) const
{
  return contest_points (m_->specialOp_, distanceKm);
}

/* CE3TSK: the one distance implementation. azdist is a spheroidal geodesic on the Clarke
   1866 ellipsoid; a spherical approximation differs by up to half a percent, tens of km at
   DX distances, which would move QSOs across the 3000 km point boundaries.

   fourCharOnly is what a contest needs: the exchange carries 4 characters, so scoring from
   a 6 character home grid would compute a distance the other station cannot reproduce from
   what we sent him. azdist itself needs no change - a blank padded 4 character grid is
   already resolved as "ll55", the centre of subsquare ll, which sits 2.5' west and 1.25'
   south of the true square centre. Both endpoints shift the same way so the error largely
   cancels: FF46 to BK29 differs by 0.5 km from a true centre calculation. */
QString contest_grid (QString const& grid, bool fourCharOnly)
{
  return fourCharOnly ? grid.left (4) : grid;
}

/* CE3TSK: filename stem for the contest's own log. Exhaustive switch, no default. */
QString contest_log_slug (Configuration::SpecialOperatingActivity activity)
{
  switch (activity)
    {
    case Configuration::SpecialOperatingActivity::WW_DIGI: return QString {"ww-digi"};
    case Configuration::SpecialOperatingActivity::NONE:
    case Configuration::SpecialOperatingActivity::NA_VHF:
    case Configuration::SpecialOperatingActivity::EU_VHF:
    case Configuration::SpecialOperatingActivity::FIELD_DAY:
...
/* The period that separates one running of a contest from the next, at the granularity that
   contest uses: a year for WW Digi, and "yyyy-MM" or an ISO week for a monthly or weekly one.
   Taken from the QSO date so that it does not depend on when contest mode was toggled.

   A contest crossing 31 December would split across two files. WW Digi runs in August so it
   cannot arise; this is the one function that would need a per contest anchor if such a
   contest is ever added. */
QString contest_period (Configuration::SpecialOperatingActivity activity, QDateTime const& when)
{
  QDateTime const utc = when.toUTC ();
  switch (activity)
    {
    case Configuration::SpecialOperatingActivity::WW_DIGI:
      return QString::number (utc.date ().year ());
    case Configuration::SpecialOperatingActivity::NONE:
    case Configuration::SpecialOperatingActivity::NA_VHF:
    case Configuration::SpecialOperatingActivity::EU_VHF:
    case Configuration::SpecialOperatingActivity::FIELD_DAY:
...
QString contest_log_filename (Configuration::SpecialOperatingActivity activity, QDateTime const& when)
{
  QString const slug = contest_log_slug (activity);
  QString const period = contest_period (activity, when);
  if (slug.isEmpty () || period.isEmpty ()) return QString {};
...
/* CE3TSK: while a contest is selected this hands out the contest list. Five of the seven
   callers in MainWindow ask at the point of use and so follow with no change; only the band
   combo's model and the LiveFrequencyValidator capture the pointer, and refreshSpecialOp()
   rebinds those two. Swapping only the combo would leave best_working_frequency() and the
   band change path on the everyday list - the dropdown and the radio disagreeing. */
FrequencyList_v2 * Configuration::frequencies ()
{
  return m_->specialOp_ != SpecialOperatingActivity::NONE ? &m_->contest_frequencies_ : &m_->frequencies_;
}
FrequencyList_v2 const * Configuration::frequencies () const
{
  return m_->specialOp_ != SpecialOperatingActivity::NONE ? &m_->contest_frequencies_ : &m_->frequencies_;
}
/* the everyday list, whatever the activity - the scheduler and anything else that must not
   follow a contest */
FrequencyList_v2 * Configuration::base_frequencies () {return &m_->frequencies_;}
...
{
#if WSJT_TRACE_CAT
  qDebug () << "Configuration::transceiver_audio:" << on << m_->cached_rig_state_;
#endif
...
{
#if WSJT_TRACE_CAT
  qDebug () << "Configuration::transceiver_tune:" << on << m_->cached_rig_state_;
#endif
...
void Configuration::transceiver_modulator_start(unsigned symbolslength, double framespersymbol, double trfrequency,
                     double tonespacing, bool synchronize, double dbsnr, double trperiod)
{
#if WSJT_TRACE_CAT
...
Configuration::impl::impl (Configuration * self, QNetworkAccessManager * network_manager, QSettings * settings, QWidget * parent)
  ...
  , contest_frequencies_ {&bands_}          /* CE3TSK */
  , next_contest_frequencies_ {&bands_}     /* CE3TSK */
  , stations_ {&bands_}
  , next_stations_ {&bands_}
  , current_offset_ {0}
  , current_tx_offset_ {0}
  , frequency_dialog_ {new FrequencyDialog {&regions_, &modes_, this}}
  /* CE3TSK: the contest table and its insert dialog see only FT8 and FT4. The proxy is set
     up in the constructor body, where modes_ is a live source model. */
  , contest_frequency_dialog_ {nullptr}
  ...
  , default_audio_output_device_selected_ {false}
{
  ...
  contest_modes_.setSourceModel (&modes_);
  contest_modes_.setFilterKeyColumn (0);
  /* only the two modes a digital contest runs. Anchored, so "FT8" cannot also match a
     hypothetical "XFT8" later. */
  contest_modes_.setFilterRegularExpression (QRegularExpression {"^(FT8|FT4)$"});
  contest_frequency_dialog_ = new FrequencyDialog {&regions_, &contest_modes_, this};

  ui_->contest_frequencies_table_view->setModel (&next_contest_frequencies_);
  ui_->contest_frequencies_table_view->horizontalHeader ()->setSectionResizeMode (QHeaderView::ResizeToContents);
  ui_->contest_frequencies_table_view->verticalHeader ()->setSectionResizeMode (QHeaderView::ResizeToContents);
  ui_->contest_frequencies_table_view->sortByColumn (FrequencyList_v2::frequency_column, Qt::AscendingOrder);
  ui_->contest_frequencies_table_view->setColumnHidden (FrequencyList_v2::frequency_mhz_column, true);
  ui_->contest_frequencies_table_view->setColumnHidden (FrequencyList_v2::mode_frequency_mhz_column, true);
  ui_->contest_frequencies_table_view->setItemDelegateForColumn (FrequencyList_v2::frequency_column, new FrequencyDelegate {this});
  ui_->contest_frequencies_table_view->setItemDelegateForColumn (FrequencyList_v2::region_column, new ForeignKeyDelegate {&regions_, 0, this});
  ui_->contest_frequencies_table_view->setItemDelegateForColumn (FrequencyList_v2::mode_column, new ForeignKeyDelegate {&contest_modes_, 0, this});

  contest_frequency_delete_action_ = new QAction {tr ("&Delete"), ui_->contest_frequencies_table_view};
  ui_->contest_frequencies_table_view->insertAction (nullptr, contest_frequency_delete_action_);
  connect (contest_frequency_delete_action_, &QAction::triggered, this, &Configuration::impl::delete_contest_frequencies);

  contest_frequency_insert_action_ = new QAction {tr ("&Insert ..."), ui_->contest_frequencies_table_view};
  ui_->contest_frequencies_table_view->insertAction (nullptr, contest_frequency_insert_action_);
  connect (contest_frequency_insert_action_, &QAction::triggered, this, &Configuration::impl::insert_contest_frequency);

  contest_load_frequencies_action_ = new QAction {tr ("&Load ..."), ui_->contest_frequencies_table_view};
  ui_->contest_frequencies_table_view->insertAction (nullptr, contest_load_frequencies_action_);
  connect (contest_load_frequencies_action_, &QAction::triggered, this, &Configuration::impl::load_contest_frequencies);

  contest_save_frequencies_action_ = new QAction {tr ("&Save as ..."), ui_->contest_frequencies_table_view};
  ui_->contest_frequencies_table_view->insertAction (nullptr, contest_save_frequencies_action_);
  connect (contest_save_frequencies_action_, &QAction::triggered, this, &Configuration::impl::save_contest_frequencies);

  contest_merge_frequencies_action_ = new QAction {tr ("&Merge ..."), ui_->contest_frequencies_table_view};
  ui_->contest_frequencies_table_view->insertAction (nullptr, contest_merge_frequencies_action_);
  connect (contest_merge_frequencies_action_, &QAction::triggered, this, &Configuration::impl::merge_contest_frequencies);
...
//  ui_->labMyCall->setVisible(!newCQZ_ && !newITUZ_ && !newDXCC_ && !newGrid_ && !newCall_);
  ...
  next_contest_frequencies_.frequency_list (contest_frequencies_.frequency_list ());   /* CE3TSK */
  ...
  next_specialOp_ = specialOp_;
  apply_special_op_lock (specialOp_ == SpecialOperatingActivity::WW_DIGI);
...
//check scheduler settings for consistency
  ...
  /* CE3TSK: the contest list. Seeded the first time with the published WW Digi frequencies
     (contest_default_frequencies(), the same list the "Contest defaults" button applies) -
     the one contest implemented, and the list an operator switching it on expects to see in
     the band selector: six FT8 and six FT4 rows, not every FT row of the everyday list.
     The everyday FT8/FT4 rows remain the fallback should that list ever be empty. After
     that it is the operator's to edit, and it persists separately. */
  {
    auto const cv = settings_->value ("ContestFrequencies");
    if (cv.isValid () && cv.canConvert<FrequencyList_v2::FrequencyItems> ())
      {
        contest_frequencies_.frequency_list (cv.value<FrequencyList_v2::FrequencyItems> ());
      }
    else
      {
        auto seed = contest_default_frequencies (SpecialOperatingActivity::WW_DIGI);
        if (seed.isEmpty ()) seed = ft_only (frequencies_.frequency_list ());
        contest_frequencies_.frequency_list (seed);
      }
  }
  ...
  {
    int stored = settings_->value ("SpecialOpActivity", -1).toInt ();
    if (stored < 0) {
      /* read_settings() runs inside SettingsGroup "Configuration", so step out of it to
         reach the old top level key and step back in. The pair is nesting balanced: the
         SettingsGroup destructor closes the group this beginGroup() reopens. */
      settings_->endGroup ();
      bool const legacy = settings_->value ("Common/WWDigiContest", false).toBool ();
      settings_->beginGroup ("Configuration");
      stored = legacy ? static_cast<int> (SpecialOperatingActivity::WW_DIGI)
                      : static_cast<int> (SpecialOperatingActivity::NONE);
    }
    if (stored < 0 || stored > static_cast<int> (SpecialOperatingActivity::ARRL_DIGI)) stored = 0;
    specialOp_ = static_cast<SpecialOperatingActivity> (stored);
  }
  /* CE3TSK: SpecialOpGridSaved was the flag of the first interlock, which parked only the
  ...
  /* CE3TSK: the parked set, read after every setting it shadows so that a key absent from
     ...
     the forced set while the operator is already in a contest: SpecialOpSaved is true, so
     accept() will not capture again, and a struct default would be handed back on leaving
     the contest in place of the value actually in use. */
...
void Configuration::impl::write_settings ()
{
  ...
  settings_->setValue ("ContestFrequencies", QVariant::fromValue (contest_frequencies_.frequency_list ()));   /* CE3TSK */
...
void Configuration::impl::accept ()
{
  ...
  /* CE3TSK: commit the special operating activity and the settings it owns. Placed after
     ...
     overwritten by a later read of a widget. Entering a contest parks the operator's own
     values, leaving one hands them back, and specialOpSaved_ stops a second entry capturing
     the forced values and making the restore a no-op. next_specialOp_ is maintained by the
     radio slots and reset by initialize_models(), the same staging the rest of this dialog
     uses. */
  SpecialOperatingActivity const chosen = next_specialOp_;
  if (chosen != SpecialOperatingActivity::NONE) {
    if (!specialOpSaved_) {
...
/* CE3TSK: while a contest owns the grid highlighting, show the settings it requires and
   stop the operator editing them. WW Digi scores the 2 character field once per band, so
   per band is forced on and per band-and-mode forced off: FT4 and FT8 count toward the same
   multiplier, and matching on mode as well would score the same field twice. */
void Configuration::impl::apply_special_op_lock (bool locked)
{
  if (locked) {
    ...
    ui_->autolog_check_box->setChecked (true);
    ui_->clear_DX_check_box->setChecked (true);
    ui_->distance_in_comments_check_box->setChecked (true);
  ...
  ui_->prompt_to_log_check_box->setEnabled (!locked);
  ui_->log_as_RTTY_check_box->setEnabled (!locked);
  ui_->report_in_comments_check_box->setEnabled (!locked);
  /* CE3TSK: the scheduler's master switch. Its own combo boxes stay on the everyday
     frequency list, so it must not run while a contest is using a different one. */
  ui_->UseSched_check_box->setEnabled (!locked);
  /* CE3TSK: the contest frequency table is the opposite way round from everything else in
     this function - it is FOR the contest, so it is editable while one is selected and
     greyed otherwise. Disabling the group box also blocks the context menu actions. */
     ...
     green plus a green dot. An invalid QColor and an empty QIcon reset to normal. */
  {
    ...
    auto * bar = ui_->configuration_tabs->tabBar ();
    /* darker green for the text than the dot, for contrast against a light tab bar */
    bar->setTabTextColor (tab, locked ? QColor {0, 64, 0} : QColor {});
    if (locked)
      {
        QPixmap dot {12, 12};
        ...
        ui_->configuration_tabs->setTabIcon (tab, QIcon {dot});
      }
    else ui_->configuration_tabs->setTabIcon (tab, QIcon {});
  }
  /* and the everyday Working Frequencies table the other way round: while a contest is
     using its own list, edits to the everyday one would silently take effect only after the
     contest - greying says so. The table view alone, not its group box: the Frequency
     Calibration spin boxes are nested inside that box and are rig properties a contest has
...
void Configuration::impl::on_rbSpecialOpNone_toggled(bool checked)
{
  if (!checked) return;
  ...
  ui_->prompt_to_log_check_box->setChecked (dlgEntrySpecialOp_.promptToLog);
...
void Configuration::impl::on_rbSpecialOpWWDigi_toggled(bool checked)
{
  if (!checked) return;
...
void Configuration::impl::on_bandComboBox_1_currentTextChanged(QString const& text)
{
  if (!text.isEmpty ()){
      ...
      ui_->hhComboBox_2->setCurrentText (sched_hh_2_);
      /* CE3TSK: not while a contest holds the scheduler down - this slot fires whenever the
         first scheduler band changes, so without the guard editing a row hands it back. */
      if (specialOp_ == SpecialOperatingActivity::NONE) {
      ...
      if (!sched_band_1_.isEmpty ()){
        ui_->hhComboBox_2->setEnabled(true);
        ui_->hhComboBox_2->setCurrentText (sched_hh_2_);
        /* CE3TSK: not while a contest holds the scheduler down */
        if (specialOp_ == SpecialOperatingActivity::NONE) {
          ui_->UseSched_check_box->setEnabled(true);
          ui_->UseSched_check_box->setChecked (usesched_);
        }
...
/* CE3TSK: keep only the two modes a digital contest runs. Applied wherever items can enter
   the contest list other than through the restricted editor. */
FrequencyList_v2::FrequencyItems
Configuration::impl::ft_only (FrequencyList_v2::FrequencyItems const& in)
{
  FrequencyList_v2::FrequencyItems out;
  for (auto const& item : in)
    if (Modes::FT8 == item.mode_ || Modes::FT4 == item.mode_) out << item;
  return out;
}

/* CE3TSK: the published default frequencies of a contest, worldwide so region ALL.
   Exhaustive switch and no default, so a contest added without deciding its frequencies
   draws -Wswitch; an empty list is a valid answer and the button reports it. */
FrequencyList_v2::FrequencyItems
Configuration::impl::contest_default_frequencies (SpecialOperatingActivity activity)
{
  FrequencyList_v2::FrequencyItems list;
  switch (activity)
    {
    case SpecialOperatingActivity::WW_DIGI:
      /* default_ true is not decoration: best_working_frequency() considers only rows with
         it set (FrequencyList.cpp:307,329), and that is what makes an FT8/FT4 mode change
         snap the band selector to the matching contest frequency. */
      for (auto f : {1840000ull, 3580000ull, 7080000ull, 14080000ull, 21080000ull, 28080000ull})
        list << FrequencyList_v2::Item {Frequency {f}, Modes::FT4, IARURegions::ALL, true};
      for (auto f : {1844000ull, 3590000ull, 7090000ull, 14090000ull, 21090000ull, 28090000ull})
        list << FrequencyList_v2::Item {Frequency {f}, Modes::FT8, IARURegions::ALL, true};
      break;
    case SpecialOperatingActivity::NONE:
    case SpecialOperatingActivity::NA_VHF:
    case SpecialOperatingActivity::EU_VHF:
    case SpecialOperatingActivity::FIELD_DAY:
    case SpecialOperatingActivity::RTTY:
    case SpecialOperatingActivity::ARRL_DIGI:
      break;
    }
  return list;
}

void Configuration::impl::set_contest_default_frequencies ()
{
  /* next_specialOp_, not specialOp_: the button acts on the contest selected in the dialog,
     committed or not, like everything else the radio buttons preview */
  auto const list = contest_default_frequencies (next_specialOp_);
  if (list.isEmpty ())
    {
      JTDXMessageBox::information_message (this, tr ("Contest defaults"),
                                           tr ("No default frequencies are defined for the selected contest."));
      return;
    }
  JTDXMessageBox msgbox;
  msgbox.setWindowTitle (tr ("Contest defaults"));
  msgbox.setIcon (JTDXMessageBox::Question);
  msgbox.setText (tr ("Are you sure you want to discard your current contest frequencies and "
                      "replace them with the selected contest's defaults?"));
  ...
  if (JTDXMessageBox::Yes == msgbox.exec ())
    {
      next_contest_frequencies_.frequency_list (list);
      ui_->contest_frequencies_table_view->resizeColumnToContents (FrequencyList_v2::mode_column);
    }
}

void Configuration::impl::delete_contest_frequencies ()
{
  auto selection_model = ui_->contest_frequencies_table_view->selectionModel ();
  selection_model->select (selection_model->selection (), QItemSelectionModel::SelectCurrent | QItemSelectionModel::Rows);
  next_contest_frequencies_.removeDisjointRows (selection_model->selectedRows ());
  ui_->contest_frequencies_table_view->resizeColumnToContents (FrequencyList_v2::mode_column);
}

void Configuration::impl::insert_contest_frequency ()
{
  /* the dialog is built against contest_modes_, so it can only offer FT8 and FT4 */
  if (QDialog::Accepted == contest_frequency_dialog_->exec ())
    {
      ui_->contest_frequencies_table_view->setCurrentIndex (next_contest_frequencies_.add (contest_frequency_dialog_->item ()));
      ui_->contest_frequencies_table_view->resizeColumnToContents (FrequencyList_v2::mode_column);
    }
}

void Configuration::impl::load_contest_frequencies ()
{
  QFileDialog* fileDlg = new QFileDialog (this);
  fileDlg->setWindowTitle (tr ("Load Contest Frequencies"));
  ...
  QString file_name = "";
  if (fileDlg->exec ()) file_name = fileDlg->selectedFiles ()[0];
  delete fileDlg;
  if (!file_name.isEmpty ())
    {
      auto const list = ft_only (read_frequencies_file (file_name));
      if (list.size ()
          && (!next_contest_frequencies_.frequency_list ().size ()
              || JTDXMessageBox::Yes == JTDXMessageBox::query_message (this
                                                               , tr ("Replace Contest Frequencies")
                                                               , tr ("Are you sure you want to discard your current "
                                                                     "contest frequencies and replace them with the "
                                                                     "loaded ones?"))))
        {
          next_contest_frequencies_.frequency_list (list);
        }
...
void Configuration::impl::save_contest_frequencies ()
{
  auto file_name = QFileDialog::getSaveFileName (this, tr ("Save Contest Frequencies"), data_dir_.absolutePath (), tr ("Frequency files (*.qrg);;All files (*.*)"));
  if (!file_name.isNull ())
    {
      ...
      ods << qrg_magic << qrg_version << next_contest_frequencies_.frequency_list ();
    }
}

void Configuration::impl::merge_contest_frequencies ()
{
  QFileDialog* fileDlg = new QFileDialog (this);
  fileDlg->setWindowTitle (tr ("Merge Contest Frequencies"));
  fileDlg->setFileMode (QFileDialog::ExistingFile);
  fileDlg->setNameFilter (tr ("Frequency files (*.qrg);;All files (*.*)"));
  fileDlg->setDirectory (data_dir_.absolutePath ());
  fileDlg->setLabelText (QFileDialog::Reject, tr ("Cancel"));
  ...
  if (!file_name.isEmpty ())
    {
      next_contest_frequencies_.frequency_list_merge (ft_only (read_frequencies_file (file_name)));
    }
}

void Configuration::impl::reset_contest_frequencies ()
{
  JTDXMessageBox msgbox;
  msgbox.setWindowTitle (tr ("Reset Contest Frequencies"));
  msgbox.setIcon (JTDXMessageBox::Question);
  /* Reset re-seeds from the everyday list rather than calling reset_to_defaults(), which
     would pull in the stock JT65 and JT9 rows a contest list must not hold. */
  msgbox.setText (tr ("Are you sure you want to discard your current contest frequencies and "
                      "replace them with the FT8 and FT4 entries from the working frequencies?"));
  msgbox.setStandardButtons (JTDXMessageBox::Yes | JTDXMessageBox::No);
  msgbox.setDefaultButton (JTDXMessageBox::No);
  msgbox.translate_buttons ();
  if (JTDXMessageBox::Yes == msgbox.exec ())
...
void Configuration::impl::transceiver_modulator_start (unsigned symbolslength, double framespersymbol, double frequency, double tonespacing, bool synchronize, double dbsnr, double trperiod)
{
  cached_rig_state_.online (true); // we want the rig online
  set_cached_mode ();
