subroutine tonesd(msgd,lcq,nthr)
  ...
  integer, intent(in) :: nthr   ! CE3TSK: this slice's table set
...
!    complex csyncsd(0:18,32),csyncsdcq(0:71,32)
  ...
  if(lcq) then
    ...
    do i=1,75
      ...
      msgsd76(i,nthr)=msg37
      i3=-1; n3=-1
      call genft8sd(msg37,i3,n3,msgsent37,msgbits,itone)
      if(i.eq.1) itone1=itone
      idtone76(i,1:29,nthr)=itone(8:36)
      idtone76(i,30:58,nthr)=itone(44:72)
      itone76(i,1:79,nthr)=itone(1:79)
    enddo
      msg37='                                     '
      msg37=trim(c1)//' '//trim(c2)//' '//trim(grid)
      msgsd76(76,nthr)=msg37
      i3=-1; n3=-1
      call genft8sd(msg37,i3,n3,msgsent37,msgbits,itone)
      idtone76(76,1:29,nthr)=itone(8:36)
      idtone76(76,30:58,nthr)=itone(44:72)
      itone76(76,1:79,nthr)=itone(1:79)
  endif
  ...
  if(lcq) then
    ...
    do i=0,57
      ...
      do j=1,32; csyncsdcq(i,j,nthr)=csig0(m); m=m+60; enddo
    enddo
  else
    call gen_ft8wave(itone1,79,1920,2.0,12000.0,0.0,csig0,xjunk,1,151680)
    do i=0,18
      do j=1,32; csyncsd(i,j,nthr)=csig0(m); m=m+60; enddo
    enddo
  endif
