...
subroutine ft8mfcq(s8,itone,msgd,msg37,lft8sd)
  ...
  integer id1(58)   ! CE3TSK: the candidate's own tone row - was written into the SHARED
                    ! idtone25(1,:) from every slice concurrently
  ...
  do k=1,25
    psum=0.; ref=ref0
    do j=1,58
      if(k.eq.1) then; i3=id1(j); else; i3=idtone25(k,j); endif
      psum=psum + s8d(i3,j); if(i3.eq.mrs(j)) ref=ref - s8d(i3,j) + s8d(mrs2(j),j)
    enddo
    p=psum/ref
