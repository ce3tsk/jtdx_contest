...
! Downconvert to complex data sampled at 200 Hz ==> 32 samples/symbol
  ...
  equivalence (x,cx)
  save cxx
! CE3TSK: the long-FFT cache is per thread. Without this a SAVE variable is one static
! shared by every decoder thread, so a thread could read the spectrum another thread had
! just written from a differently subtracted dd8 - one source of unrepeatable results.
