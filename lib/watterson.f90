...
!
! CE3TSK: nsig was missing from this argument list while ft8sim, ft8sim_gfsk and ft4sim
! all pass it, so every argument after it was shifted by one place: fs received NWAVE,
! delay received fs and fspread received delay. Any simulation asking for fading or for
! multipath delay produced a waveform that no decoder could read.
...
! CE3TSK: with no delay there is no second path, and the shift is to the right
...
!3001 format(3f12.6)
  enddo
  rms=sqrt(sq/nsig) ! CE3TSK: normalise over the signal, not over the whole padded vector
  c=c/rms
