...
subroutine ft4_baseline(s,nfa,nfb,sbase,nfe)
! CE3TSK item 65: nfe - the bin the fitted baseline is EVALUATED to, beyond the fit span
! nfa..nfb (getcandidates4 fits over JTDX's 200-4910 Hz as before, so nothing below 4910 Hz
! changes, and evaluates to 5000 Hz so the candidate search can reach the top of the band
! as FT8's does).
...
! Output: sbase(npts)    Baseline
  ...
  ib=min(NH1,nfb); ie=min(NH1,max(ib,nfe))   ! CE3TSK item 65
  do i=ia,ib
  ...
  do i=ia,ie   ! CE3TSK item 65: evaluated to ie, fitted over ia..ib
     t=i-i0
