...
! CE3TSK: source-level switch, not exposed in the GUI or the ini. Type 3 (ARRL RTTY Roundup
! exchange, "W9XYZ K1ABC R 579 MA") is no longer worked in FT8/FT4 on the air, so every
! type-3 decode we see is a false one (random serial and report behind a CRC hit, ~10 a
! month at -19 dB and below). With the switch off unpack77 rejects the type; set it to
! .true. to decode RTTY Roundup exchanges again (the pack side, i.e. transmitting them from
! the RTTY special activity, is unaffected).
  logical, parameter :: LRTTYRU_DECODE=.false.
! CE3TSK: the same for type 0.5 telemetry (18 hex digits, "1564C7B008B8AD4BAC"): nobody sends
! FT8 telemetry on the HF bands, and a random 71-bit codeword behind a CRC hit prints as
! exactly that (~50 a month, nearly all at -20 dB and below). Set to .true. to decode it.
  logical, parameter :: LTELEMETRY_DECODE=.false.
  ...
  character (len=13), dimension(1:4800) :: last_calls=''   ! CE3TSK: 48 slices x 100
...
! 0.5   0123456789abcdef01                 71               71   Telemetry (18 hex)
     if(.not.LTELEMETRY_DECODE) unpk77_success=.false.   ! CE3TSK: see the switch at the top of the module
     read(c77,1006) ntel
...
! Type 3: ARRL RTTY Contest
     if(.not.LRTTYRU_DECODE) unpk77_success=.false.   ! CE3TSK: see the switch at the top of the module
