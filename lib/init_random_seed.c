...
void init_random_seed(void)
{
  ...
  /* CE3TSK: JTDX_SIM_SEED=n makes the simulators (ft8sim, ft4sim, ...) reproducible - the
     regression tests need the same noise every run; unset, the seed is random as before */
  char const * fixed = getenv ("JTDX_SIM_SEED");
  if (fixed && *fixed)
    {
      srand ((unsigned) strtoul (fixed, NULL, 10));
      return;
    }
