...
subroutine ft8mf1(s8,itone,msgd,msg37,lft8sd,nthr)

  use ft8_mod1, only : itone76,idtone76,msgsd76
  integer, intent(in) :: nthr   ! CE3TSK: this slice's tonesd tables
  ...
  do k=1,76
    psum=0.; ref=ref0
    do j=1,58; i3=idtone76(k,j,nthr); psum=psum + s8d(i3,j); if(i3.eq.mrs(j)) ref=ref - s8d(i3,j) + s8d(mrs2(j),j); enddo
    p=psum/ref
...
!      if(trim(msgsd76(ipk)).eq.'CQ DE AA00') return ! triggered by memory corruption?
      lft8sd=.true.; msg37=msgsd76(ipk,nthr); itone(1:79)=itone76(ipk,1:79,nthr); return
    endif
  endif
