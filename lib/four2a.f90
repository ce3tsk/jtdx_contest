...
! CE3TSK: the lookup above runs unlocked in every thread. The entry used to be published
! (nplan incremented) before its plan pointer existed, so a thread looking up the same size
! and array at that moment could match the entry and execute a plan that was not there yet -
! a rare first-decode race with 12 threads starting together. Now: re-check under the lock,
! fill the slot completely, create the plan, and only then publish it.
     found_plan=.false.
     do i=1,nplan
        if(nfft.eq.nn(i) .and. isign.eq.ns(i) .and. iform.eq.nf(i) .and. nloc.eq.nl(i)) then
           found_plan=.true.
           exit
        endif
     enddo
     if(.not.found_plan) then
     i=nplan+1
     if(i.ge.NPMAX) stop 'Too many FFTW plans requested.'   ! re-checked under the lock
