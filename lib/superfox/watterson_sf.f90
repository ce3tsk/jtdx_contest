subroutine watterson_sf(c,npts,nsig,fs,delay,fspread)

! CE3TSK: WSJT-X 3.0.2 lib/ft8/watterson.f90 under another name, for sfoxsim only. jtdx has its
! own watterson (lib/watterson.f90, stock JTDX's) whose Doppler spectrum is exp(-(f/(fspread/2))**2)
! where this one is exp(-(f/fspread)**2): the same "fspread" gives half the spread there. The
! SuperFox figures published by the WSJT-X team (SuperFox_Performance.pdf: 50 % decode near
! -15.3 dB on the "MM" channel) were made with this routine, so the SuperFox simulator keeps it
! and its files stay comparable with the donor's sfoxsim and with the paper. Only the name is
! changed; jtdx's FT8/FT4/FT2 simulators keep theirs, as their recorded thresholds were made
! with it.
!
