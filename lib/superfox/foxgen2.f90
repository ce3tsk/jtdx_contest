...
! CE3TSK: jtdx's split77 sets the length of the LAST word only (nw(nwords)); WSJT-X's sets
! every nw(i), and this routine reads nw(1), nw(3) and nw(4). Without the loop below the
! Fox call came out of w(4)(2:nw(4)-1) with a stray length and was sent as a hash, and
! Hounds were dropped. The only change to the donor's foxgen2.
     do j=1,nwords
        nw(j)=len(trim(w(j)))
     enddo
     ntype=0
