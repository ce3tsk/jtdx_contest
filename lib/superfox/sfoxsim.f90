...
! CE3TSK: from WSJT-X 3.0.2 lib/superfox/sfoxsim.f90, the test signal source of the SuperFox
! receiver (SUPERFOX_PLAN.md). jtdx does not transmit SuperFox; this tool exists to make audio.
! What differs from the donor:
! - sgran() seeds the noise, so JTDX_SIM_SEED=n makes the files reproducible as it does for the
!   other simulators; without it the noise differs from run to run, as in the donor;
! - an optional 11th argument, the 6-digit one-time code put in the signature field. The donor
!   always sends 000000 (unsigned), so no signed test file could be made with it;
! - the channel simulator is the donor's, carried as watterson_sf (see watterson_sf.f90).
  ...
  nargs=iargc()
  if(nargs.ne.10 .and. nargs.ne.11) then
     print*,'Usage:   sfoxsim   f0  DT Chan FoxC H1 H2 CQ FT nfiles snr [otp]'
     ...
     print*,'         FT=1 to include a Free Text message (read from text_msg.txt)'
     print*,'         otp  optional 6-digit one-time code; default 000000 = unsigned'
     go to 999
  endif
  call getarg(1,arg)
  read(arg,*) f0
  ...
  if(nargs.eq.11) then
     call getarg(11,arg)
     read(arg,*) notp                     !checks that it is a number
     write(ckey,'("OTP:",i6.6)') notp      !sfox_pack reads the code from ckey(5:10)
  endif
...
! Generate cdat, the SuperFox waveform
     ...
     if(fspread.ne.0 .or. delay.ne.0) call watterson_sf(crcvd,NMAX,NZ,fsample,&
          delay,fspread)
