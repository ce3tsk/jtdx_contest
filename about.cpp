  ...
  ui(new Ui::CAboutDlg)
{
  ...
  /* CE3TSK: the Ko-fi wordmark is artwork, not text - the words are drawn into the PNG by
     tools/make_support_button.py. So the catalogue does not translate the words here, it names
     the file: each language translates this tag to its own, e.g. "support_es_ES", and the
     matching support_<tag>_{dark,light}.png is generated by that script and globbed into the
     resources by CMakeLists.txt. Left untranslated it stays "support", the English pair, which
     is the right fallback. */
  //: Artwork tag, not a word to translate: give the tag of this language's Ko-fi button,
  //: e.g. support_es_ES, and generate it with tools/make_support_button.py. Leave it as
  //: "support" and the English button is used.
  QString const wordmark {tr ("support")};
